...
export default function DashboardPage() {
  ...
  const kpis = currentAnalysis
    ? [
        ...
        { icon: BarChart3, label: "Articles Presse", value: currentAnalysis.sources.press, color: "from-purple-500 to-pink-500", explanation: "Articles collectés depuis les flux RSS de la presse marocaine (Hespress, Le360, Médias24, etc.).", items: toItems(currentAnalysis.articles.filter((a) => !a.isUpload)) },
        { icon: FileText, label: "Documents Uploadés", value: currentAnalysis.sources.uploads, color: "from-indigo-500 to-violet-500", explanation: "Documents importés manuellement (page Upload) et dont le texte a été intégré au calcul des mentions, partis et alertes.", items: toItems(currentAnalysis.articles.filter((a) => a.isUpload)) },
        { icon: Globe, label: "Mentions Sociales", value: currentAnalysis.sources.social, color: "from-amber-500 to-yellow-500", explanation: "Aucune source de réseaux sociaux n'est connectée pour le moment (architecture \"RSS presse uniquement\"). Ce chiffre reste à 0 tant qu'aucune API sociale n'est intégrée.", items: [] },
      ]
    : [];
