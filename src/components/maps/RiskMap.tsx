...
import type { RegionRisk } from "@/stores/useAnalysisStore";
...
export function RiskMap({ risks, onSelect }: { risks: RegionRisk[]; onSelect?: (r: RegionRisk) => void }) {
  ...
  return (
    <GlassCard className="p-6">
      <div className="mb-4">
        ...
        <p className="text-sm text-gray-400">Répartition géographique des alertes par région (basée sur les articles réels analysés)</p>
      </div>
      ...
      {risks.length === 0 ? (
        <p className="text-sm text-gray-500 py-8 text-center">Aucune alerte régionale détectée dans cette analyse.</p>
      ) : (
        <svg viewBox="0 0 800 550" className="w-full h-auto">
          ...
          {risks.map((r) => {
            ...
            const radius = Math.max(r.alertCount * 4, 12);
            const isHovered = hovered === r.id;
            const color = riskColors[r.riskLevel];

            return (
              <g key={r.id} onMouseEnter={() => setHovered(r.id)} onMouseLeave={() => setHovered(null)} onClick={() => onSelect?.(r)} className="cursor-pointer">
                <motion.circle
